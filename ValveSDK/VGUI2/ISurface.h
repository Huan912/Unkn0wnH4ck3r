...
namespace vgui
{
	...
	public:
		...
		virtual void DrawUnicodeCharAdd( wchar_t wch ) = 0;//20
		...
		virtual void SetTitle( IPanel* panel , const wchar_t *title ) = 0;//40
		...
		virtual void LockCursor( void ) = 0;//55
		...
		virtual bool AddGlyphSetToFont( HFont font , const char *windowsFontName , int tall , int weight , int blur , int scanlines , int flags , int lowRange , int highRange ) = 0; //67?????
		...
		virtual void PlaySound( const char *fileName ) = 0; //68
